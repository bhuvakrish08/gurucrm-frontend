...
const emptyForm = {
  ...
  assignee: "", priority: "Medium", status: "Pending",
};
...
function StatusCheckbox({ status, onClick, size = "md" }) {
  ...
  if (status === "Completed") {
    return (
      ...
      >
        <svg className={iconDims} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function ActivityModal({ open, onClose, editData, users, onSaved }) {
  ...
  useEffect(() => {
    if (editData && !editData._prefillDate) {
      setForm({
        ...
        assignee:      editData.assignee ? String(editData.assignee) : "",
        ...
        status: STATUSES.includes(editData.status) ? editData.status : "Pending",
      });
    } else {
      // ✅ FIX 1: Navo event banta j status hammesha "Pending"
      setForm({ ...emptyForm, activity_date: editData?._prefillDate || todayStr(), status: "Pending" });
    }
    setError("");
  }, [editData, open]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editData && !editData._prefillDate) {
        await api.put(`/calendar/${editData.id}`, form);
      } else {
        // ✅ navo event banti vakhte status force "Pending" mokli e
        await api.post("/calendar/insert-event", { ...form, status: "Pending" });
      }
      onSaved();
      onClose();
    } catch (err) {
    ...
    <Modal open={open} onClose={onClose}>
      <div className="p-6 bg-white">
        <div className="flex items-center justify-between mb-5">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-orange-500 text-2xl leading-none transition-colors">&times;</button>
        </div>
            ...
            <div>
              ...
              {isEdit ? (
                ...
                <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2 bg-gray-50">
                  ...
                  <span className="text-sm text-gray-500">Pending (default)</span>
                </div>
              )}
            </div>
          </div>
    ...
    <Modal open={open} onClose={onClose}>
      <div className="p-6 text-center bg-white">
        ...
        <p className="text-sm text-gray-500 mb-6">
          <span className="font-medium text-gray-700">"{title}"</span> permanently delete thase. Aa action undo nahi thay.
        </p>
...
// DETAIL MODAL
// ============================================================
function DetailModal({ open, onClose, event, onEdit, onDelete, onStatusChange }) {
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 pr-4">
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {new Date(toLocalDateStr(event.activity_date) + "T00:00:00").toLocaleDateString("en-GB", {
                weekday: "long", day: "numeric", month: "long", year: "numeric"
              })}
              {" · "}{fmt12(event.activity_time)}
            </p>
          </div>
          <button onClick={onClose} className="text-gray-400 hover:text-orange-500 text-2xl leading-none mt-0.5 transition-colors">&times;</button>
        </div>

        {event.description && (
          <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3 mb-4">{event.description}</p>
        )}

        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-500 mb-1">Priority</p>
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${PRIORITY_STYLE[event.priority] || "bg-gray-100 text-gray-600"}`}>
              {event.priority || "—"}
            </span>
          </div>
          <div className="bg-gray-50 rounded-lg p-3">
            <p className="text-xs text-gray-500 mb-1">Assignee</p>
            <p className="text-sm font-medium text-gray-700">{event.assignee_name || event.assignee || "—"}</p>
          </div>
        </div>

        {/* ✅ FIX: Status dropdown/pills hatavi, checkbox sathe replace karyu. Click karta cycle thay: Pending -> Completed -> Cancelled */}
        <div className="mb-5">
          <p className="text-xs text-gray-500 mb-2">Status — click box to change</p>
          <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-3">
            <StatusCheckbox
              status={event.status}
              onClick={() => onStatusChange(event.id, nextStatus(event.status))}
            />
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${STATUS_STYLE[event.status] || "bg-gray-100 text-gray-600"}`}>
              {event.status}
            </span>
          </div>
        </div>
...
export default function CalendarPage() {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      const msg = err.response?.status === 401
        ? "Session expire thayu. Please re-login karo."
        : "Events load nahi thaya. Backend check karo.";
      showToast(msg, "error");
    } finally {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      const res = await api.get("/manage-user/list");
      setUsers(res.data.data || res.data || []);
    } catch {
      // Users na aave to text input show thase
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast("Activity delete thayi.");
  ...
  const handleStatusChange = async (id, status) => {
    ...
    setDetailEvent((prev) => (prev && prev.id === id ? { ...prev, status } : prev));
    try {
      await api.put(`/calendar/status/${id}`, { status });
      showToast(`Status "${status}" set thayu.`);
    } catch (err) {
  ...
  const eventsOnDate = (dateStr) =>
    events.filter((e) => toLocalDateStr(e.activity_date) === dateStr);
  ...
  const selectedEvents = eventsOnDate(selectedDate).sort(
    (a, b) => (a.activity_time || "").localeCompare(b.activity_time || "")
  );
  ...
  return (
    // ✅ FIX: Akhu calendar white background par
    <div className="min-h-screen bg-white">

      {/* TOAST */}
      {toast && (
        <div className={`fixed top-4 right-4 z-[100] px-4 py-3 rounded-xl shadow-lg text-sm font-medium text-white transition-all
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                <button onClick={prevMonth} className="p-2 hover:bg-gray-100 rounded-lg text-gray-600 hover:text-orange-500 transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7"/>
                  </svg>
                </button>
                <div className="flex items-center gap-3">
                  <h2 className="font-semibold text-gray-900">{MONTHS[viewMonth]} {viewYear}</h2>
                  <button onClick={goToday} className="text-xs text-orange-500 hover:text-orange-600 hover:underline">Today</button>
                </div>
                <button onClick={nextMonth} className="p-2 hover:bg-gray-100 rounded-lg text-gray-600 hover:text-orange-500 transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                      ...
                      <div className="space-y-0.5">
                        {dayEvts.slice(0, 2).map((ev) => (
                          ...
                          >
                            <span
                              onClick={(ev2) => { ev2.stopPropagation(); handleStatusChange(ev.id, nextStatus(ev.status)); }}
                              className="flex-shrink-0"
                            >
                              <StatusCheckbox status={ev.status} onClick={() => {}} size="sm" />
                            </span>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                ...
                <button
                  onClick={() => { setEditData({ _prefillDate: selectedDate }); setShowForm(true); }}
              ...
              <div className="p-4 overflow-y-auto max-h-[500px]">
                {loadingData ? (
                  ...
                  <div className="text-center py-10">
                    ...
                    <p className="text-xs text-gray-300 mt-1">Click + to add one</p>
                  </div>
                ) : (
                    ...
                    {selectedEvents.map((ev) => (
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          ...
                          <span onClick={(e) => { e.stopPropagation(); handleStatusChange(ev.id, nextStatus(ev.status)); }} className="mt-0.5">
                            <StatusCheckbox status={ev.status} onClick={() => {}} />
                          </span>
                          <div className="flex-1" onClick={() => setDetailEvent(ev)}>
                            <div className="flex items-start justify-between gap-2">
                              <p className="text-sm font-medium text-gray-800 group-hover:text-orange-600 leading-snug">{ev.title}</p>
                              <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full flex-shrink-0 ${STATUS_STYLE[ev.status] || "bg-gray-100 text-gray-600"}`}>
                                {ev.status}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 mt-1.5">
                              <span className="text-xs text-gray-500">{fmt12(ev.activity_time)}</span>
                              {ev.priority && (
                                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${PRIORITY_STYLE[ev.priority] || ""}`}>
                                  {ev.priority}
                                </span>
                              )}
                            </div>
        ...
        {viewMode === "list" && (
          <div>
            ...
            {loadingData ? (
              <div className="text-center py-16 text-gray-400">Loading activities...</div>
            ) : listEvents.length === 0 ? (
                ...
                {listEvents.map((ev, idx) => (
                  ...
                  >
                    ...
                    <div className="col-span-1">
                      <StatusCheckbox
                        status={ev.status}
                        onClick={() => handleStatusChange(ev.id, nextStatus(ev.status))}
                      />
                    </div>
                    ...
                    <div className="col-span-2">
                      <p className="text-xs text-gray-700">
                        {new Date(toLocalDateStr(ev.activity_date) + "T00:00:00").toLocaleDateString("en-GB", { day: "numeric", month: "short" })}
                      </p>
                      <p className="text-xs text-gray-400">{fmt12(ev.activity_time)}</p>
                    </div>
                    <div className="col-span-2">
                      <p className="text-xs text-gray-600 truncate">{ev.assignee_name || ev.assignee || "—"}</p>
                    </div>
                    <div className="col-span-1">
                      {ev.priority ? (
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${PRIORITY_STYLE[ev.priority] || ""}`}>
                          {ev.priority}
                        </span>
                      ) : <span className="text-gray-300 text-xs">—</span>}
                    </div>
                    <div className="col-span-2">
                      <span className={`text-xs px-2 py-1 rounded-lg font-medium ${STATUS_STYLE[ev.status] || "bg-gray-100 text-gray-600"}`}>
                        {ev.status}
                      </span>
                    </div>
                    <div className="col-span-1 flex items-center gap-1.5">
                      <button
                        onClick={() => { setEditData(ev); setShowForm(true); }}
      ...
      <ActivityModal
        ...
        onSaved={() => {
          fetchEvents();
          showToast(editData && !editData._prefillDate ? "Activity update thayi!" : "Activity create thayi!");
        }}
      />
      ...
      <DetailModal
        ...
        event={detailEvent}
        onEdit={(ev) => { setEditData(ev); setShowForm(true); }}
