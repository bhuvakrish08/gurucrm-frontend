...
export default function ProformaPage() {
  ...
  const [splitForm, setSplitForm] = useState({
    amount_9: 0, amount_18: 0,
    tax_percent_9: "9.00", tax_percent_18: "18.00",
    tax_9: "0.00", tax_18: "0.00",
    grand_total: "0.00", amount: 0,
  });
  ...
  const [filters, setFilters] = useState({
    customer_name: "", assignee: "", status: "", quotation_no: "",
    from_date: "", to_date: "", min_percentage: "", max_percentage: "",
    min_total: "", max_total: "",
  });
  ...
  useEffect(() => {
    const fetchAssignee = async () => {
      try {
        const res = await axios.get(`${API}/api/manage-user/asignee`, { params: { status: 1 } });
  ...
  const formatPINumber = (index) => {
    ...
    let nextYear = year + 1;
    if (date.getMonth() < 3) { year = year - 1; nextYear = year + 1; }
  ...
  const updateStatus = async (pi_id, newTotalPercentage) => {
    let newStatus = newTotalPercentage >= 100 ? "paid" : newTotalPercentage > 0 ? "partial" : "draft";
    try {
  ...
  const getGrandTotal = (pi) => {
    if (pi.follow_ups && pi.follow_ups.length > 0) {
      ...
      if (f.proforma_percentage > 0) return (f.total / f.proforma_percentage) * 100;
    }
    if (pi.proforma_percentage > 0) return (pi.total / pi.proforma_percentage) * 100;
    return pi.total || 0;
  };
  ...
  const downloadInvoicePDF = async (item, index) => {
    try {
      ...
      const historyA = sortedFollowUps.filter((f) => Number(f.proforma_percentage_9 || 0) > 0);
      const historyB = sortedFollowUps.filter((f) => Number(f.proforma_percentage_18 || 0) > 0);

      const { base9, base18 } = getBaseAmounts(item);
      const grandTotal = getGrandTotal(item);

      const latestFollowUp = sortedFollowUps[0];
      const totalPaid = latestFollowUp ? Number(latestFollowUp.total || 0) : Number(item.total || 0);
      ...
      const statusLabel =
        item.status === "paid" ? "WON / PAID" :
        item.status === "partial" ? "PENDING" :
        item.status === "sent" ? "SENT" :
        item.status === "cancelled" ? "CANCELLED" : "DRAFT";
  ...
  useEffect(() => { setCurrentPage(1); }, [filters, itemsPerPage, activeTab]);
  ...
  const pendingCount   = piData.filter((d) => String(d.stage || "pending").trim().toLowerCase() !== "completed").length;
  const completedCount = piData.filter((d) => String(d.stage || "").trim().toLowerCase() === "completed").length;
      ...
      <div className="bg-gray-100 min-h-screen">
            ...
            <div className="relative w-full sm:w-auto" ref={exportRef}>
              ...
              {showExportMenu && (
                <div className="absolute right-0 top-full mt-1.5 w-44 bg-white rounded-sm shadow-lg border border-gray-100 overflow-hidden z-50">
                  <button onClick={exportToExcel} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 transition-all text-left">
                    <i className="bi bi-file-earmark-excel text-green-600 text-base"></i> Export Excel
                  </button>
                  <div className="h-px bg-gray-100 mx-3"></div>
                  <button onClick={exportToPDF} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 hover:bg-red-50 hover:text-red-700 transition-all text-left">
        ...
        <div className={`${showMobileFilters ? "absolute left-6 right-6 top-[170px] bg-white p-5 shadow-2xl rounded-lg grid grid-cols-2 gap-3 mt-1 z-[999] ring-2 ring-orange-300" : "hidden"} md:mx-6 md:flex md:flex-wrap md:items-center md:gap-x-3 md:gap-y-2 md:mt-3 md:mb-5 md:relative md:bg-transparent md:p-0 md:shadow-none md:ring-0`}>
          ...
          <select name="assignee" value={filters.assignee} onChange={handleFilterChange} className="p-2 w-full md:w-45 bg-white border border-orange-300 md:border rounded-sm focus:outline-none text-gray-400 text-sm">
            ...
            {assigneeList.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}
          </select>
          ...
          <div className="flex gap-2 col-span-2 md:col-span-1">
            <button onClick={() => { resetFilters(); setShowMobileFilters(false); }} className="border border-gray-300 w-full md:w-auto cursor-pointer rounded-sm p-2 bg-gray-200 text-gray-700 hover:bg-gray-300 text-sm font-semibold text-center px-6">Clear</button>
            <button onClick={() => setShowMobileFilters(false)} className="md:hidden border border-orange-300 w-full cursor-pointer rounded-sm p-2 bg-orange-100 text-orange-700 hover:bg-orange-200 text-sm font-semibold text-center px-6">Apply</button>
          </div>
        </div>

        {/* TABS */}
        <div className="mx-7 mt-2 mb-0 flex items-center gap-0 border-b border-gray-200 bg-white px-2 pt-2 rounded-t-sm">
          <button onClick={() => { setActiveTab("pending"); setCurrentPage(1); }} className={`px-5 py-2.5 text-sm font-semibold transition-all relative rounded-t-md ${activeTab === "pending" ? "text-blue-600 border-b-2 border-blue-500 bg-white" : "text-gray-400 hover:text-gray-600 border-b-2 border-transparent"}`}>
            Pending
            <span className={`ml-2 text-xs px-2 py-0.5 rounded-full font-bold ${activeTab === "pending" ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-400"}`}>{pendingCount}</span>
          </button>
          <button onClick={() => { setActiveTab("completed"); setCurrentPage(1); }} className={`px-5 py-2.5 text-sm font-semibold transition-all relative rounded-t-md ${activeTab === "completed" ? "text-green-600 border-b-2 border-green-500 bg-white" : "text-gray-400 hover:text-gray-600 border-b-2 border-transparent"}`}>
            Completed
            <span className={`ml-2 text-xs px-2 py-0.5 rounded-full font-bold ${activeTab === "completed" ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}>{completedCount}</span>
          </button>
        </div>
              ...
              <div className="overflow-x-auto overflow-y-scroll max-h-[500px] custom-scroll" style={{ overflowX: "scroll" }}>
                <table className="w-full text-sm whitespace-nowrap">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-100">
                      {["#", "PI No", "PI Date", "Customer Name", "Quotation No", "Source", "Reference", "Assignee", "Total", "PI %", "Status", "Stage", "Follow-Up", "Download"].map((h) => (
                        <th key={h} className="py-3 px-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedData.length > 0 ? paginatedData.map((item, index) => {
                      ...
                      return (
                        <tr key={item.pi_id} className="border-b border-gray-50 hover:bg-indigo-50/30 transition-colors">
                          ...
                          <td className="py-3 px-3 text-gray-500">{item.pi_date ? new Date(item.pi_date).toLocaleDateString("en-IN") : "-"}</td>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-slate-200 bg-white">
                  <div className="flex items-center gap-3">
                    ...
                    <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm text-slate-700 bg-white focus:outline-none cursor-pointer font-medium">
                      {[10, 20, 100, 200].map((size) => <option key={size} value={size}>{size}</option>)}
                    </select>
                  </div>
                  {totalPages > 1 && (
                    <div className="flex items-center gap-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white w-full max-w-[980px] rounded-2xl shadow-2xl border border-gray-100 overflow-hidden max-h-[95vh] overflow-y-auto">
                  ...
                  <div className="space-y-1 text-sm mb-4">
                    ...
                    {activePartTab === "a" ? (
                      <div className="flex justify-between py-1.5">
                        <span className="text-gray-400 text-xs font-semibold uppercase tracking-wide">Part A Base (Other Charges, incl. tax)</span>
                        <span className="font-semibold text-orange-500">Rs.{Number(base9).toLocaleString("en-IN")}</span>
                      </div>
                    ) : (
                      <div className="flex justify-between py-1.5">
                        <span className="text-gray-400 text-xs font-semibold uppercase tracking-wide">Part B Base (Project Value, incl. tax)</span>
                        <span className="font-semibold text-blue-600">Rs.{Number(base18).toLocaleString("en-IN")}</span>
                      </div>
                    )}
                  </div>
                ...
                <div className="w-full md:w-1/2 px-6 py-5 flex flex-col bg-gray-50/50">
                  <div className="flex justify-between items-center mb-4">
                    <p className="text-xs font-bold text-gray-600 uppercase tracking-widest">
                      {activePartTab === "a" ? "Follow-Up History — Other Charges" : "Follow-Up History — Project Value"}
                    </p>
                      ...
                      shownHistory.map((h, index) => {
                        ...
                        return (
                          <div key={h.id}>
                            ...
                            {activeIndex === index && (
                              <div className={`mt-2 border rounded-xl p-4 text-sm shadow-sm ${
                                activePartTab === "a"
                                  ? "border-orange-200 bg-gradient-to-br from-orange-50 to-white"
                                  : "border-blue-200 bg-gradient-to-br from-blue-50 to-white"
                              }`}>
                                <div className="flex justify-between items-center mb-3">
                                  <p className={`font-bold text-xs uppercase tracking-wide ${activePartTab === "a" ? "text-orange-500" : "text-blue-500"}`}>Details</p>
                                  <button onClick={() => setActiveIndex(null)} className="text-gray-400 hover:text-gray-600 text-xs">✕ Close</button>
                                </div>
                                <div className="grid grid-cols-1 gap-3">
                                  {activePartTab === "a" ? (
                                    <div className="bg-orange-50 border border-orange-100 rounded-lg p-3">
                                      <p className="text-xs font-bold text-orange-500 mb-2 uppercase">Part A — Other Charges</p>
                                      <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                          <span className="text-gray-400">Percentage</span>
                                          <span className="font-semibold text-gray-700">{pct9v.toFixed(2)}%</span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                          <span className="text-gray-400">Amount</span>
                                          <span className="font-semibold text-gray-700">Rs.{Number(t9v).toLocaleString("en-IN")}</span>
                                        </div>
                                      </div>
                                    </div>
                                  ) : (
                                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
                                      <p className="text-xs font-bold text-blue-500 mb-2 uppercase">Part B — Project Value</p>
                                      <div className="space-y-1">
                                        <div className="flex justify-between text-xs">
                                          <span className="text-gray-400">Percentage</span>
                                          <span className="font-semibold text-gray-700">{pct18v.toFixed(2)}%</span>
                                        </div>
                                        <div className="flex justify-between text-xs">
                                          <span className="text-gray-400">Amount</span>
                                          <span className="font-semibold text-gray-700">Rs.{Number(t18v).toLocaleString("en-IN")}</span>
                                        </div>
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  <div className="bg-white border border-gray-100 rounded-lg p-3">
                                    <div className="flex justify-between text-xs">
                                      ...
                                      <span className="font-semibold text-gray-700">{Number(h.proforma_percentage || 0).toFixed(2)}%</span>
                                    </div>
                                    <div className="flex justify-between text-xs mt-1">
                                      <span className="text-gray-400">Total Amount</span>
                                      <span className="font-semibold text-gray-700">Rs.{Number(h.total || 0).toLocaleString("en-IN")}</span>
                                    </div>
                                    <div className="flex justify-between text-xs mt-1">
                                      <span className="text-gray-400">Date</span>
                                      <span className="font-semibold text-gray-700">{new Date(h.created_at).toLocaleDateString("en-IN")}</span>
                                    </div>
                                  </div>
                                </div>
                              </div>
              ...
              <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
                <button onClick={resetModal} className="px-5 py-2 rounded-xl text-sm font-medium border border-gray-200 text-gray-600 hover:bg-gray-100 transition-all">Cancel</button>
                <button
        ...
        <div className="fixed inset-0 z-[65] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white w-[500px] rounded-lg shadow-2xl overflow-hidden border border-gray-100 flex flex-col text-gray-800">
            <div className="flex justify-between items-center px-6 py-4 bg-gradient-to-r from-orange-100 to-white border-b border-gray-100">
              ...
              <button onClick={() => setShowSplitModal(false)} className="w-8 h-8 rounded-full hover:bg-gray-100 transition-colors flex items-center justify-center text-orange-500 border-0 bg-transparent cursor-pointer">✕</button>
            </div>
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-left">
                    <div className="bg-blue-50/20 p-4 rounded-xl border border-blue-100 space-y-3">
                      <div><label className="text-[11px] font-bold text-blue-700 uppercase tracking-wide block mb-1">Project Value (₹)</label><div className="border border-blue-200 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">₹ {Number(splitForm.amount_18 || 0).toLocaleString()}</div></div>
                      <div><label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide block mb-1">Split (%)</label><div className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">{percent18}%</div></div>
                      <div><label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide block mb-1">Tax Rate (%)</label><div className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">{splitForm.tax_percent_18}%</div></div>
                      <div className="mt-2.5 space-y-1 text-xs text-gray-500 pt-2 border-t border-dashed border-gray-200">
                        <div className="flex justify-between"><span>Tax ({splitForm.tax_percent_18}%):</span><span className="font-medium text-gray-700">₹ {splitForm.tax_18}</span></div>
                        <div className="flex justify-between border-t border-dashed border-gray-150 pt-1"><span>Total B:</span><span className="font-bold text-gray-700">₹ {(parseFloat(splitForm.amount_18 || 0) + parseFloat(splitForm.tax_18 || 0)).toFixed(2)}</span></div>
                      </div>
                    </div>
                    <div className="bg-orange-50/20 p-4 rounded-xl border border-orange-100 space-y-3">
                      <div><label className="text-[11px] font-bold text-orange-700 uppercase tracking-wide block mb-1">Other Charges (₹)</label><div className="border border-orange-200 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">₹ {Number(splitForm.amount_9 || 0).toLocaleString()}</div></div>
                      <div><label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide block mb-1">Split (%)</label><div className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">{percent9}%</div></div>
                      <div><label className="text-[11px] font-bold text-gray-500 uppercase tracking-wide block mb-1">Tax Rate (%)</label><div className="border border-gray-300 rounded-md px-3 py-1.5 text-sm bg-white font-semibold text-gray-800 shadow-sm">{splitForm.tax_percent_9}%</div></div>
                      <div className="mt-2.5 space-y-1 text-xs text-gray-500 pt-2 border-t border-dashed border-gray-200">
                        <div className="flex justify-between"><span>Tax ({splitForm.tax_percent_9}%):</span><span className="font-medium text-gray-700">₹ {splitForm.tax_9}</span></div>
                        <div className="flex justify-between border-t border-dashed border-gray-150 pt-1"><span>Total A:</span><span className="font-bold text-gray-700">₹ {(parseFloat(splitForm.amount_9 || 0) + parseFloat(splitForm.tax_9 || 0)).toFixed(2)}</span></div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-6 py-4 bg-gray-50 flex justify-end border-t border-gray-100 rounded-b-lg">
              <button type="button" onClick={() => setShowSplitModal(false)} className="px-5 py-2 rounded-lg text-sm font-semibold bg-gray-200 text-gray-700 hover:bg-gray-300 transition-all cursor-pointer border-0">Close</button>
            </div>
          </div>
        </div>
      )}
