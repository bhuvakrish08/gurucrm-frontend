...
function Page() {
  ...
  const getProjectExpenseTimeSeries = () => {
    ...
    items.forEach(item => {
      ...
      if (viewModalTimeframe === 'weekly') {
        ...
        label = sunday.toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })
  ...
  const getProjectExpenseCategories = () => {
    ...
    viewExpenses.forEach(exp => {
      ...
      categories[cat] = (categories[cat] || 0) + (Number(exp.expense_amount) || 0)
    })

    const totalArch = viewArchitectures.reduce((sum, arch) => sum + (Number(arch.architecture_amount) || 0), 0)
    if (totalArch > 0) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/project/analytics/dashboard`, {
  ...
  const handleManageArchitecture = async (project) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_BASE_URL}/api/project/${project.id}/architectures`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
      ...
      if (!res.ok) {
        throw new Error(`Failed to load project architectures (Status ${res.status})`)
      }
  ...
  const addArchitect = (architectId) => {
    ...
    const arch = allArchitects.find(a => String(a.id) === String(architectId))
    if (!arch) return

    // Check if already assigned
    if (assignedArchitects.some(a => String(a.id) === String(arch.id) || a.architecture_name === arch.name)) {
      return
    }
  ...
  const handleSaveArchitectures = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_BASE_URL}/api/project/${selectedProject.id}/architectures`, {
        method: 'POST',
        headers: {
  ...
  const handleManageExpenses = async (project) => {
    ...
    try {
      ...
      const catRes = await fetch(`${API_BASE_URL}/api/expense-category/read?status=1`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (catRes.ok) {
      ...
      const expRes = await fetch(`${API_BASE_URL}/api/project/${project.id}/expenses`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (!expRes.ok) {
        throw new Error(`Failed to load project expenses (Status ${expRes.status})`)
      }
  ...
  const handleSaveExpenses = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_BASE_URL}/api/project/${selectedProject.id}/expenses`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleViewProject = async (project) => {
    ...
    try {
      ...
      const archRes = await fetch(`${API_BASE_URL}/api/project/${project.id}/architectures`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (archRes.ok) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_BASE_URL}/api/project/update/${selectedProject.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleDeleteProject = async (projectId) => {
    if (!window.confirm("Are you sure you want to delete this project? This will permanently remove the project along with all assigned architecture commissions and expenses.")) {
      return
    }

    try {
      const token = localStorage.getItem('token')
      const res = await fetch(`${API_BASE_URL}/api/project/delete/${projectId}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`
        }
    ...
    <div>
      ...
      <div className="p-4 md:p-8">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-slate-800">Project Workspace</h1>
            <p className="text-xs text-slate-500 mt-1">
              {viewMode === 'list'
                ? (loading ? 'Loading projects...' : `${projects.length} active project records`)
          ...
          <div className="space-y-6">
            {loadingAnalytics && !analyticsData ? (
              <div className="flex flex-col items-center justify-center py-24 gap-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                ...
                <p className="text-sm font-semibold text-slate-500 animate-pulse">Assembling financial dashboard...</p>
              </div>
            ) : analyticsError ? (
              <div className="p-6 bg-rose-50 border border-rose-100 rounded-2xl text-center">
                ...
                <h3 className="text-sm font-bold text-rose-800">Failed to load analytics dashboard</h3>
                <p className="text-xs text-rose-600 mt-1">{analyticsError}</p>
                <button onClick={fetchAnalytics} className="mt-4 px-4 py-2 bg-rose-600 text-white rounded-xl text-xs font-bold hover:bg-rose-700 transition-colors cursor-pointer">
                  Try Again
                </button>
              </div>
            ) : analyticsData ? (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                  ...
                  <div className="bg-gradient-to-br from-white to-indigo-50/40 hover:to-indigo-50/80 rounded-2xl border border-slate-100 p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md relative overflow-hidden group">
                    ...
                    <div>
                      ...
                      <h2 className="text-2xl font-extrabold text-slate-800 mt-0.5">{analyticsData.stats.total_projects}</h2>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-white to-orange-50/40 hover:to-orange-50/80 rounded-2xl border border-slate-100 p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md relative overflow-hidden group">
                    ...
                    <div>
                      ...
                      <h2 className="text-2xl font-extrabold text-slate-800 mt-0.5">{formatCurrency(analyticsData.stats.total_revenue)}</h2>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-white to-amber-50/40 hover:to-amber-50/80 rounded-2xl border border-slate-100 p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md relative overflow-hidden group">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Architecture Net</span>
                      <h2 className="text-2xl font-extrabold text-slate-800 mt-0.5">{formatCurrency(analyticsData.stats.total_architecture_net)}</h2>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-white to-rose-50/40 hover:to-rose-50/80 rounded-2xl border border-slate-100 p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md relative overflow-hidden group">
                    ...
                    <div>
                      ...
                      <h2 className="text-2xl font-extrabold text-slate-800 mt-0.5">{formatCurrency(analyticsData.stats.total_expense_net)}</h2>
                    </div>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-white to-emerald-50/40 hover:to-emerald-50/80 rounded-2xl border border-slate-100 p-5 flex flex-col justify-between shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md relative overflow-hidden group">
                    ...
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Net Revenue</span>
                      <h2 className="text-2xl font-extrabold text-emerald-600 mt-0.5">{formatCurrency(analyticsData.stats.total_net_revenue)}</h2>
                    </div>
                  </div>
                </div>
                ...
                {(() => {
                  const monthlyFinancialsParsed = (analyticsData.monthlyFinancials || []).map(m => ({
                    ...m,
                  ...
                  const architectureByArchitectParsed = (analyticsData.architectureByArchitect || []).map(item => ({
                    ...item,
                          ...
                          <div className="h-80 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                              ...
                              >
                                ...
                                <XAxis dataKey="month_name" stroke="#94a3b8" fontSize={11} tickLine={false} />
                                <YAxis stroke="#94a3b8" fontSize={11} tickLine={false} />
                                <Tooltip
                                  contentStyle={{ background: '#fff', borderRadius: '12px', border: '1px solid #f1f5f9', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.05)' }}
                                  formatter={(value) => formatCurrency(value)}
                                />
                                <Legend wrapperStyle={{ fontSize: 11, paddingTop: 10 }} />
                                <Bar dataKey="revenue" name="Grand Total" fill="#4f46e5" radius={[4, 4, 0, 0]} maxBarSize={20} />
                                <Bar dataKey="expenses" name="Expenses" fill="#f43f5e" radius={[4, 4, 0, 0]} maxBarSize={20} />
                                <Bar dataKey="net_revenue" name="Net Revenue" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={20} />
                              </BarChart>
                            </ResponsiveContainer>
                          </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 flex flex-col justify-between">
                            ...
                            {expenseByCategoryParsed.length === 0 ? (
                              ...
                              <>
                                ...
                                <div className="absolute flex flex-col items-center justify-center">
                                  ...
                                  <span className="text-sm font-extrabold text-slate-800">{formatCurrency(totalExpensesSum)}</span>
                                </div>
                              </>
                            )}
                          </div>
                          ...
                          <div className="max-h-24 overflow-y-auto space-y-1 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100/50 custom-scroll mt-2">
                            {expenseByCategoryParsed.map((cat, idx) => {
                              const COLORS = ['#f43f5e', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6', '#06b6d4', '#64748b'];
                              return (
                                <div key={idx} className="flex items-center justify-between text-[11px] font-semibold text-slate-600">
                                  <div className="flex items-center gap-1.5">
                                    <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }}></span>
                                    <span className="truncate max-w-[120px]">{cat.expense_category}</span>
                                  </div>
                                  <span className="text-slate-800">{formatCurrency(cat.total_amount)}</span>
                                </div>
                              )
                            })}
                          </div>
                            ...
                            {expensesOverTimeParsed.length === 0 ? (
                              ...
                              <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                                  <XAxis
                                    dataKey={expenseTimeframe === 'weekly' ? 'week_start' : (expenseTimeframe === 'monthly' ? 'month_name' : 'year_key')}
                                    stroke="#94a3b8"
                                    fontSize={10}
                                    tickLine={false}
                                  />
                                  ...
                                  <Tooltip
                                    ...
                                    formatter={(value) => formatCurrency(value)}
                                  />
                                  <Bar dataKey="amount" name="Expense Cost" fill="#f43f5e" radius={[4, 4, 0, 0]} maxBarSize={30} />
                                </BarChart>
                              </ResponsiveContainer>
                            )}
                          </div>
                        ...
                        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                          ...
                          <div className="h-64 w-full">
                            {architectureByArchitectParsed.length === 0 ? (
                              <div className="flex items-center justify-center h-full">
                                <p className="text-xs text-slate-400 font-medium">No architecture commissions assigned.</p>
                              </div>
                            ) : (
                              <ResponsiveContainer width="100%" height="100%">
                                <BarChart
                                ...
                                >
                                  ...
                                  <YAxis dataKey="architecture_name" type="category" stroke="#94a3b8" fontSize={10} tickLine={false} width={100} />
                                  <Tooltip
                                    contentStyle={{ background: '#fff', borderRadius: '12px', border: '1px solid #f1f5f9' }}
                                    formatter={(value) => formatCurrency(value)}
                                  />
                                  <Bar dataKey="total_amount" name="Total Commission" fill="#f59e0b" radius={[0, 4, 4, 0]} maxBarSize={20} />
                                </BarChart>
                              </ResponsiveContainer>
                            )}
                          </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
                  ...
                  {analyticsData.topProjects.length === 0 ? (
                    <p className="text-xs text-slate-400 font-medium py-4 text-center">No projects found.</p>
                  ) : (
                              ...
                              <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="py-2.5 px-4">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-500 w-8">{marginPct}%</span>
                                  </div>
                                </td>
                              </tr>
                            )
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 transition-all duration-300 animate-fadeIn">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] transform scale-100 transition-all duration-300 ease-out">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-5 flex items-center justify-between text-white shadow-md">
              <div>
                <h2 className="text-xl font-bold tracking-tight">Assign Architecture Commission</h2>
                <p className="text-xs text-orange-50 opacity-90 mt-0.5">
                  Project: {selectedProject?.company_name || '-'} | Customer: {selectedProject?.customer_name || '-'}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 custom-scroll">
              {modalError && (
                <div className="bg-rose-50 border-l-4 border-rose-500 p-4 rounded-r-xl flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-rose-900">Error Occurred</h4>
              ...
              {loadingArch ? (
                <div className="flex flex-col items-center justify-center py-16 space-y-3">
                  ...
                  <p className="text-sm text-slate-500 font-medium">Fetching assigned architectures...</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                      <i className="bi bi-people-fill text-orange-500"></i> Assigned Architectures ({assignedArchitects.length})
                    </h3>

                    {assignedArchitects.length === 0 ? (
                      <div className="text-center py-8 border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                        <i className="bi bi-person-dash text-3xl text-slate-300 block mb-2"></i>
                        <p className="text-xs text-slate-400 font-medium">No architects assigned to this project yet.</p>
                      </div>
                    ) : (
                        ...
                        <table className="w-full text-sm text-left text-slate-600">
                          <thead className="bg-slate-50 text-[11px] uppercase tracking-wider font-semibold text-slate-500 border-b border-slate-100">
                            <tr>
                              ...
                              <th className="py-2.5 px-4 w-36">Commission %</th>
                              <th className="py-2.5 px-4 w-40">Commission Amount</th>
                                  ...
                                  <td className="py-3 px-4">
                                    <div className="relative flex items-center">
                                      <input
                                        ...
                                        max="100"
                                        step="0.01"
                                        value={arch.percentage === 0 || arch.percentage === "0" || arch.percentage === "0.00" ? "" : arch.percentage}
                                        placeholder="0"
                                      ...
                                      />
                                      <span className="absolute right-2 text-slate-400 text-xs font-semibold">%</span>
                                    </div>
                                  </td>
                                  <td className="py-3 px-4">
                                    <div className="relative flex items-center">
                                      ...
                                      <input
                                        ...
                                        min="0"
                                        step="0.01"
                                        value={arch.architecture_amount === 0 || arch.architecture_amount === "0" || arch.architecture_amount === "0.00" ? "" : arch.architecture_amount}
                                        placeholder="0"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-amber-500 px-6 py-5 text-white relative">
              <h2 className="text-xl font-bold tracking-tight">Manage Project Expenses</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              {loadingExpense ? (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <p className="text-sm text-slate-500 font-medium">Loading project expenses...</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="grid grid-cols-5 divide-x divide-slate-100 border border-slate-100 rounded-2xl bg-slate-50/50 p-4 shadow-inner">
                    ...
                    <div className="text-center p-1">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Architecture Net</span>
                      <p className="text-sm font-bold text-slate-600 mt-1">
                        {formatCurrency(selectedProject?.architecture_net_amount)}
                      </p>
                    </div>
                    <div className="text-center p-1">
                      <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Expense Net</span>
                      <p className="text-sm font-bold text-orange-600 mt-1">
                        {formatCurrency(
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          min="0.01"
                          step="0.01"
                          value={newExpenseAmount === 0 || newExpenseAmount === "0" || newExpenseAmount === "" ? "" : newExpenseAmount}
                          placeholder="0"
                  ...
                  <div className="space-y-3">
                    <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                      <i className="bi bi-receipt-cutoff text-orange-500"></i> Project Expense List ({projectExpenses.length})
                    </h3>

                    {projectExpenses.length === 0 ? (
                      <div className="text-center py-8 border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                        <i className="bi bi-wallet2 text-3xl text-slate-300 block mb-2"></i>
                        <p className="text-xs text-slate-400 font-medium">No expenses added to this project yet.</p>
                      </div>
                    ) : (
                      <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm">
                        <table className="w-full text-sm text-left text-slate-600">
            ...
            <div className="border-b border-slate-100 px-6 py-4 flex items-center justify-between bg-slate-50">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Quotation No: {selectedProject.quotation_no || '-'} | {selectedProject.company_name}
                </p>
              </div>
              <button
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                          ...
                          <div className="bg-gradient-to-br from-white to-orange-50/20 hover:to-orange-50/40 rounded-xl border border-slate-100 p-4 flex flex-col justify-between shadow-sm transition-all duration-200">
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Project Value</span>
                          ...
                          <div className="bg-gradient-to-br from-white to-amber-50/20 hover:to-amber-50/40 rounded-xl border border-slate-100 p-4 flex flex-col justify-between shadow-sm transition-all duration-200">
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Architect Commissions</span>
                          ...
                          <div className="bg-gradient-to-br from-white to-rose-50/20 hover:to-rose-50/40 rounded-xl border border-slate-100 p-4 flex flex-col justify-between shadow-sm transition-all duration-200">
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Project Expenses</span>
                          ...
                          <div className="bg-gradient-to-br from-white to-emerald-50/20 hover:to-emerald-50/40 rounded-xl border border-slate-100 p-4 flex flex-col justify-between shadow-sm transition-all duration-200">
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-[10px] font-bold text-slate-400 tracking-wider uppercase">Net Revenue</span>
                              <div className="p-1.5 bg-emerald-50 rounded-lg text-emerald-600">
                                <TrendingUp size={14} />
                              </div>
                            </div>
                            <div>
                              <h3 className="text-base font-extrabold text-emerald-600">{formatCurrency(netRev)}</h3>
                              <p className="text-[9px] text-emerald-700 font-bold mt-0.5">{margin}% profit margin</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                          ...
                          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-3">
                            ...
                            <div className="h-48 w-full">
                              {getProjectExpenseTimeSeries().length === 0 ? (
                                <div className="flex items-center justify-center h-full">
                                  <p className="text-[10px] text-slate-400 font-medium">No expense items found.</p>
                                </div>
                              ) : (
                                <ResponsiveContainer width="100%" height="100%">
                                  <BarChart
                                  ...
                                  >
                                    ...
                                    <Tooltip
                                      contentStyle={{ background: '#fff', borderRadius: '8px', border: '1px solid #f1f5f9', fontSize: 10 }}
                                      formatter={(value) => formatCurrency(value)}
                                    />
                                    <Bar dataKey="amount" name="Cost" fill="#f43f5e" radius={[4, 4, 0, 0]} maxBarSize={20} />
                                  </BarChart>
                                </ResponsiveContainer>
                              )}
                            </div>
                          ...
                          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-3">
                            <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                              <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                                <PieChartIcon size={14} className="text-rose-500" />
                                Cost Distribution
                              </h4>
                            </div>
                            <div className="flex items-center justify-between h-48">
                              {getProjectExpenseCategories().length === 0 ? (
                                <div className="flex items-center justify-center w-full h-full">
                                  <p className="text-[10px] text-slate-400 font-medium">No expenses/commissions assigned.</p>
                                </div>
                              ) : (
                                <>
                                  <div className="h-full w-1/2 flex items-center justify-center relative">
                                    <ResponsiveContainer width="100%" height="100%">
                                      <PieChart>
                                        <Pie
                                          ...
                                          paddingAngle={2}
                                        >
                                          {getProjectExpenseCategories().map((entry, index) => {
                                            const COLORS = ['#f59e0b', '#f43f5e', '#3b82f6', '#10b981', '#8b5cf6', '#06b6d4', '#64748b'];
                                            return <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />;
                                          })}
                                        </Pie>
                                        <Tooltip formatter={(value) => formatCurrency(value)} />
                                    ...
                                    <div className="absolute flex flex-col items-center justify-center pointer-events-none">
                                      ...
                                      <span className="text-[10px] font-extrabold text-slate-800">
                                        {formatCurrency(getProjectExpenseCategories().reduce((sum, item) => sum + item.value, 0))}
                                      </span>
                                    </div>
                                  </div>
                                  <div className="w-1/2 max-h-40 overflow-y-auto space-y-1.5 pl-2 custom-scroll">
                                    {getProjectExpenseCategories().map((cat, idx) => {
                                      const COLORS = ['#f59e0b', '#f43f5e', '#3b82f6', '#10b981', '#8b5cf6', '#06b6d4', '#64748b'];
                                      return (
                                        <div key={idx} className="flex flex-col text-[10px] text-slate-600">
                                          <div className="flex items-center gap-1">
                                            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: COLORS[idx % COLORS.length] }}></span>
                                            <span className="truncate font-semibold max-w-[100px]">{cat.name}</span>
                                          </div>
                                          <span className="pl-3 font-bold text-slate-800">{formatCurrency(cat.value)}</span>
                                        </div>
                                      )
                                    })}
                                  </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 space-y-3">
                          ...
                          {viewArchitectures.length === 0 ? (
                            <p className="text-[10px] text-slate-400 font-medium py-3 text-center">No architectures assigned.</p>
                          ) : (
                            <div className="overflow-x-auto">
                              <table className="w-full text-xs text-left text-slate-600 border-collapse">
                                <thead className="bg-slate-50 text-[9px] uppercase font-bold text-slate-500 tracking-wider">
                                  <tr>
                                    <th className="py-2 px-3 border-b border-slate-100">Architect</th>
                                    <th className="py-2 px-3 border-b border-slate-100">Email / Contact</th>
                                    <th className="py-2 px-3 text-center border-b border-slate-100">Commission %</th>
                                    <th className="py-2 px-3 text-right border-b border-slate-100">Commission Amount</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 text-slate-700 bg-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <div>
                          ...
                          <span className="text-sm font-medium text-slate-700">{selectedProject.company_name || '-'}</span>
                        </div>
                        <div>
                          <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Customer</span>
                          <span className="text-sm font-medium text-slate-700">{selectedProject.customer_name || '-'}</span>
                        </div>
                        <div>
                          <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Reference</span>
                          <span className="text-sm font-medium text-slate-700">{selectedProject.reference || '-'}</span>
                        </div>
                        <div>
                          <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Source</span>
                          <span className="text-sm font-medium text-slate-700">{selectedProject.source || '-'}</span>
                        </div>
                        <div>
                          <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Quotation Date</span>
                          <span className="text-sm font-medium text-slate-700">{formatDate(selectedProject.quotation_date)}</span>
                        </div>
                        <div>
                          <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Created At</span>
                          <span className="text-sm font-medium text-slate-700">{formatDate(selectedProject.created_at)}</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-4">
                        ...
                        <div className="flex justify-between items-center text-sm text-rose-600">
                          ...
                          <span className="font-semibold">{formatCurrency(selectedProject.architecture_net_amount)}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm text-rose-600">
                          <span>(-) Expense Net</span>
                          <span className="font-semibold">{formatCurrency(selectedProject.expense_net_amount)}</span>
                        </div>
                        <div className="border-t border-slate-200/80 pt-3 flex justify-between items-center text-base font-bold text-slate-900">
                          <span>Net Revenue</span>
                          <span className="text-emerald-600">{formatCurrency(selectedProject.net_revenue_amount)}</span>
                        </div>
                      </div>
                    </div>
                  )}
                        ...
                        <div className="overflow-hidden rounded-xl border border-slate-100 bg-white">
                          <table className="min-w-full divide-y divide-slate-100">
                            <thead className="bg-slate-50">
                              <tr>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-slate-500">Architect Name</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider text-slate-500">Commission %</th>
                                <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider text-slate-500">Commission Amount</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 text-sm">
                              {viewArchitectures.map((arch) => (
                                <tr key={arch.id} className="hover:bg-slate-50/50">
                                  <td className="px-4 py-3 font-semibold text-slate-700">{arch.architecture_name}</td>
                                  <td className="px-4 py-3 text-center text-slate-500">{arch.percentage}%</td>
                                  <td className="px-4 py-3 text-right font-medium text-slate-700">{formatCurrency(arch.architecture_amount)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                            ...
                            <tbody className="divide-y divide-slate-100 text-sm">
                              {viewExpenses.map((exp) => (
                                <tr key={exp.id} className="hover:bg-slate-50/50">
                                  <td className="px-4 py-3 font-semibold text-slate-700">{exp.expense_category}</td>
                                  <td className="px-4 py-3 text-slate-500 max-w-[200px] truncate" title={exp.description}>{exp.description || '-'}</td>
                                  <td className="px-4 py-3 text-right font-medium text-slate-700">{formatCurrency(exp.expense_amount)}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
